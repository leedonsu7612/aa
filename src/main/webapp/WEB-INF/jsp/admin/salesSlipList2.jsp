...
<form:form commandName="searchVO" id="searchForm" name="searchForm" class="mt20" method="post">    
	...
	<div class="tab-contents">
		...
		<ul>
			...
			<li class="active"><a href="#NONE" id="tab05" onClick='gotoPage("<c:url value='/admin/salesSlipList.do'/>");'>매출전표</a></li>
		</ul>
		<!--// tab menu -->     
	</div>
	<!--// detail tabs -->
	...
	<!-- 매출전표 조회 -->
	<h3 class="hidden">매출전표 조회</h3>
    <table class="tbl_view2 mb10 mt15" summary="매출전표 검색내역입니다.">
        <caption>매출전표 조회</caption>
		<thead>
			<!-- 테이블 헤드 -->
			<tr>
				<th class="tbl_center" scope="col">No</th>
				<th class="tbl_center" scope="col">매출일</th>
				<th class="tbl_center" scope="col">헤임즈전표번호</th>
			<c:choose>
				<c:when test="${searchVO.membClass eq 'B'}">
				...
				<th class="tbl_center" scope="col">전표금액</th>
				<th class="tbl_center" scope="col">세금계산서발행여부</th>
				<th class="tbl_center" scope="col">세금계산서번호</th>
				<th class="tbl_center" scope="col">매출구분</th>
				<th class="tbl_center" scope="col">헤임즈기록보증초과금액</th>
				<th class="tbl_center" scope="col">이니시스잔여보증초과금액</th>
			</tr>
		</thead>
        ...
        <tbody>
     			...
     			<c:forEach var="result" items="${resultList}" varStatus="status">
                     <tr>
                         ...
                         <td class="text_center"><c:out value="${result.heimsSalesSlipNo}"/></td>
					<c:choose>
						<c:when test="${searchVO.membClass eq 'B'}">
						...
						<c:when test="${searchVO.membClass eq 'S'}">
                         ...
                         <td class="text_center"><fmt:formatNumber value="${result.supplyValue }" pattern="#,###" /></td>
                         <td class="text_center"><c:out value="${result.useTaxbillYn}"/></td>
                         <td class="text_center"><c:out value="${result.taxbillNo}"/></td>
                         <td class="text_center"><c:out value="${result.groupType}"/></td>
                         <td class="text_center"><fmt:formatNumber value="${result.overInput }" pattern="#,###" /></td>
                         <td class="text_center"><fmt:formatNumber value="${result.overOutput }" pattern="#,###" /></td>
                     </tr>
                 </c:forEach>
         </tbody>
    </table>
