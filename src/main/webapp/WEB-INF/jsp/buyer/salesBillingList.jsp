...
<body>
	<div id="wrap">
		...
		<div id="container" class="agency">
			<div id="contents">
				...
				<form action="<c:url value='/buyer/salesBilling/list.do'/>" method="post" name="searchForm" id="searchForm">
				...
				<div class="tbl-type01 basic mt20">
					<table summary="대리점 거래내역 상세정보 입니다.">
						<caption>대리점 거래내역 정보</caption>
						<colgroup>
							<col width="7%" />
							<col width="11%" />
							<col width="11%" />
						...
						<thead>
							<tr>
								<th scope="col">년월</th>
								<th scope="col">매입내역</th>
								<th scope="col">세금계산서<br />발행금액</th>
								<th scope="col">총지급내역</th>
								<th scope="col">만기일<br />지정지급</th>
								<th scope="col">기타지급</th>
	...
	<script>
		var EXCEL_URL = "<c:url value='/downloads/buyerSalesBilling.do'/>"; // EXCEL EXPORT URL
		var COLLECTION_BY_PERIOD_URL = "<c:url value='/buyer/salesBilling/list.json'/>"; // 기간별 결제내역조회 URL
			...
			if(dataLength > 0) {
				...
				for(i = 0; i < dataLength; i++) {
					html =
						...
						+ "<td>" + (data[i].progressFlag == 1 || data[i].progressFlag == 5 ? "<a href=\"javascript:;\" class=\"fs-red2 progressFlag" + i + "\">" + getProgressStatus(data[i].progressFlag, "B") + "</a>" : getProgressStatus(data[i].progressFlag, "B")) + "</td>"
						+ "</tr>";

					$collectionTb.append(html);

					if(data[i].progressFlag == 1 || data[i].progressFlag == 5) {
						$collectionTb.find(".progressFlag" + i).on("click", {
							taxBillNumber : data[i].taxBillNo,
							transactionNumber : data[i].transactionNo,
							paymentDate : data[i].paymentDate,
							expirationDate : data[i].expirationDate,
							amountOfMoney : data[i].amount,
							expirationAvg : data[i].expirationAvg
						}, salesDetailClick);
					}
		...
		// 구매 결제내역서, 결제내역조회 팝업
		function salesDetailClick(e, taxBillNumber, transactionNumber, paymentDate, expirationDate, amountOfMoney, expirationAvg) {
			// TODO 만기일 변경 팝업
			console.log("salesDetailClick", e);
		}
		...
		// 채권만기일 미지정 페이지로 이동
		function linkToAccountReceivable(salesDate) {
			$("#searchDate").val(salesDate);
			$("#searchForm").attr("action", "<c:url value='/buyer/accountReceivable/list.do'/>");
			submitSearchForm();
		}
		...
		$(function() {
			...
			$(document).on("click", ".collectAmount a", function() {
				...
				var buyerBizNo = "${user.businessNo}"; // 대리점 사업자번호
			...
			$(document).on("click", ".endAmount a", function(){
				...
				/* if (Number(unexpectedDue) > 0 && accessLevel != "R") { */
					var html  = "<a href=\"javascript:;\" onclick=\"linkToAccountReceivable('" + salesDate + "');\">" + comma(unexpectedDue) + "</a>";
					$("#unexpectedDue").html(html);
