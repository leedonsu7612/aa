...
<body class="main">
	...
	<div id="wrap">
		<div id="container">
			<div id="contents">
				<!-- title -->
				<h2 class="hidden">품목센터 지급 현황(종합)</h2>
				<!-- // title -->

				<!-- 검색조건 -->
				<form id="searchForm" class="mt20" action="<c:url value='/buyer/companysBilling/list.do' />" method="post">
						...
						<table class="tbl_search" summary="">
							...
							<tbody>
								<tr>
									...
									<td>
										<p>
											<label for="searchDate" class="hidden">조회 기준날짜 선택</label><input type="text" name="searchDate" id="searchDate" class="input ac mr5 EnterKeyDown" style="width: 95px;" value="${params.searchDate }">
											<img src="<c:url value='/images/ico/ico_calendar.gif'/>" alt="검색 기준날짜 선택" class="mr15" onclick="dateClick('searchDate')">
				...
				<!-- 품목지원센터 지급현황(종합) -->
				<h3 class="hidden">품목지원센터 지급현황(종합)</h3>

				<table id="ariListTable" class="tbl_view2 mb10 mt15" summary="품목지원센터 지급현황(종합)을 상세하게 조회합니다.">
					...
					<thead>
						<tr>
							<th scope="col" class="tbl_center" rowspan="2" id="store_name">품목지원센터</th>
							<th scope="col" class="tbl_center" colspan="3" id="give_month">지급 대상(조회월 기준)</th>
							<th scope="col" class="tbl_center" rowspan="2" id="this_month">당월지급</th>
							<th scope="col" class="tbl_center" rowspan="2" id="recovery">지급율</th>
							<th scope="col" class="tbl_center" rowspan="2" id="balance">미지급금</th>
							...
							<c:otherwise>
								<c:forEach var="item" items="${list}" varStatus="status">
									<c:choose>
										...
										<c:otherwise>
										<tr>
											<td class="text_center businessName" id="store_name + '${status.current }'" businessnumber="${item.sellerBizNo }">
												<a href="javascript:;">[${item.sellerMembNo }] <script>changeGroupColor('${item.sellerCompanyName }');</script></a>
											</td>
				...
				<!-- EXCEL URL -->
				<input type="hidden" id="excelUrl" value="<c:url value='/downloads/buyerCompanyBilling.do' />">
				<!-- // EXCEL URL -->
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#ariListTable > tbody > tr > .businessName").click(function(event) {
				...
				$("#sellerBizNo").val($(this).attr("businessNumber"));
				$("#startDate").val(null);
				$("#endDate").val(null);
				$("#searchForm").submit();
			});
