...
<body>
	<div id="wrap">
		...
		<div id="container" class="agency">
			<div id="contents">
				...
				<form action="<c:url value='/buyer/unauthorized/list.do'/>" method="post" name="searchForm" id="searchForm">
				...
				<div class="tbl-type01 basic">
					<table summary="기타지급 승인요청 조회 상세 현황입니다.">
						<caption>기타지급 승인 목록</caption>
						<colgroup>
							<col width="8%" />
							<col width="15%" />
							<col width="15%" />
								...
								<c:otherwise>
									<c:forEach var="item" items="${list }" varStatus="status">
										<tr>
											...
											<td>기타지급</td>
											<td>
												<span class="hide" taxbillNo="${item.taxbillNo }" transactionNo="${item.transactionNo }" amount="${item.amount }"></span>
	...
	<script>
		var EXCEL_URL = "<c:url value='/downloads/buyerUnauthorized.do'/>"; // EXCEL EXPORT URL
		...
		function updateUnauthorized(jsonData) {
			...
			if(flag == "3") { // 승인
				ajaxUrl = "<c:url value='/buyer/unauthorized/insert.json'/>";
			} else if (flag == "2") { // 취소
				ajaxUrl = "<c:url value='/buyer/unauthorized/delete.json'/>";
			} else {
				return false;
			}
		...
		$(function() {
			...
			$(document).on("click", ".paymentBtn", function(){
				...
				var jsonData = {
						taxbillNo : attributes.taxbillNo.value,
