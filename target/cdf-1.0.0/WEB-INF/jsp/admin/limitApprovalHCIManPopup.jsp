...
<%
	/**
	* @Class Name : limitApprovalHCIManPopup.jsp
	* @Description : 대리점한도요청 HCI부서원 승인팝업
...
%>
							...
							<table summary="">
								...
								<tbody>
									...
									<tr>
										...
										<td class="txt-r">
											<c:if test='${resultVO.lmtReqGbn eq "LM04"}'><fmt:formatNumber value="${resultVO.notarialPrice }" pattern="#,###" /><input type="hidden" name="notarialPrice" value="<c:out value='${resultVO.notarialPrice}'/>"/></c:if>
											<c:if test='${resultVO.lmtReqGbn ne "LM04"}'><input type="text" name="notarialPrice" placeholder="공증금액입력" class="wp100 txt-r" value="<c:out value='${resultVO.notarialPrice}'/>"/></c:if>
										</td>
									</tr>
									<tr>
									...
									<c:if test='${resultVO.lmtReqGbn ne "LM04"}'>
									<tr>
										<th scope="row">모비스 사업소</th>
										<td colspan="3">
											<div class="select-bx wp100">
												<label for="mobi-s">모비스 사업소</label>
												<select name="bizPlace">
													<option value="모비스">모비스 사업소</option>
												</select>
											</div>
										</td>
									</tr>
							...
							<table summary="">
								...
								<tbody>
									<tr>
										...
										<td>
											<input type="radio" id="processNo1" name="processNo" value="3" <c:if test='${resultVO.processNo eq "3"}'>checked</c:if>/><label for="processNo1"><span></span>확인중</label>
											<input type="radio" id="processNo2" name="processNo" value="4" <c:if test='${resultVO.processNo eq "4"}'>checked</c:if>/><label for="processNo2"><span></span>승인</label>
											<input type="radio" id="processNo3" name="processNo" value="98" <c:if test='${resultVO.processNo eq "98"}'>checked</c:if>/><label for="processNo3"><span></span>취소</label>
										</td>
									</tr>
								</tbody>
							</table>
	...
	<script type="text/javascript">
		...
		function approvalClick(){			
			if($("input[name=processNo]:checked").val() == '3'){
				$("#addLimitPopup > .msg-txt").text("한도요청을 확인 하시겠습니까?"); //div 내용 변경
			}
			if($("input[name=processNo]:checked").val() == '98'){
