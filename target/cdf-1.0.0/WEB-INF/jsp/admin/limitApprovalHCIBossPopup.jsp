...
%>
		<div>
			<form id="reqForm" name="reqForm" action="<c:url value='/admin/insertLimitRequest.json'/>" method="post">    
			    ...
			    <input type="hidden" name="notarialPrice" value="${resultVO.notarialPrice}"/>
				<ul>
					<li><span>요청내용</span>
							...
							<table summary="">
								...
								<tbody>
									<tr>
										...
										<td>
											<input type="radio" id="processNo1" name="processNo" value="6" <c:if test='${resultVO.processNo eq "6"}'>checked</c:if>/><label for="processNo1"><span></span>승인</label>
											<input type="radio" id="processNo2" name="processNo" value="5" <c:if test='${resultVO.processNo eq "5"}'>checked</c:if>/><label for="processNo2"><span></span>승인취소</label>
											<input type="radio" id="processNo3" name="processNo" value="99" <c:if test='${resultVO.processNo eq "99"}'>checked</c:if>/><label for="processNo3"><span></span>반려</label>
										</td>
									</tr>
									<tr>
										<th scope="row">비고</th>
										<td><input type="text" name="memo" class="wp100" placeholder="사유를 입력하세요" value="<c:out value='${resultVO.memo}'/>"/></td>
									</tr>
								</tbody>
							</table>
						</div>
	...
	<script type="text/javascript">
		...
		function approvalClick(){			
			if($("input[name=processNo]:checked").val() == '5'){
				$("#addLimitPopup > .msg-txt").text("한도요청을 승인취소 하시겠습니까?"); //div 내용 변경
			}
			if($("input[name=processNo]:checked").val() == '99'){
				$("#addLimitPopup > .msg-txt").text("한도요청을 반려 하시겠습니까?"); //div 내용 변경
			}
